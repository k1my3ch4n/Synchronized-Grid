...
test.describe("Real-time Collaboration", () => {
  ...
  test("two users see each other in user presence", async () => {
    ...
    await expect(
      pageA.getByPlaceholder("https://example.com"),
    ).toBeVisible({ timeout: 10000 });

    // User B 접속
    await pageB.goto(`/workspace/${workspaceId}`);
    await expect(
      pageB.getByPlaceholder("https://example.com"),
    ).toBeVisible({ timeout: 10000 });
  ...
  test("viewport add syncs between users", async () => {
    // 두 유저 접속
    await pageA.goto(`/workspace/${workspaceId}`);
    await pageB.goto(`/workspace/${workspaceId}`);

    // URL이 이미 설정되어 있으므로 캔버스 대기
    // URL 입력이 보이면 설정
    const urlInputA = pageA.getByPlaceholder("https://example.com");
    if (await urlInputA.isVisible({ timeout: 3000 }).catch(() => false)) {
      await urlInputA.fill("https://example.com");
      await pageA.getByRole("button", { name: "Load" }).click();
    }

    // 캔버스가 렌더링될 때까지 대기
    await expect(pageA.getByText("+ Add Preset")).toBeVisible({
      timeout: 10000,
    });
    await expect(pageB.getByText("+ Add Preset")).toBeVisible({
      timeout: 10000,
    });

    // User A가 뷰포트 추가
    await pageA.getByText("+ Add Preset").click();
    await pageA.getByPlaceholder("Label").fill("Sync Test");
    await pageA.getByPlaceholder("W").fill("400");
    await pageA.getByPlaceholder("H").fill("300");
    await pageA.getByRole("button", { name: "OK" }).click();

    // User A에서 뷰포트 표시 확인
    await expect(pageA.getByText("Sync Test")).toBeVisible();

    // User B에서도 동기화되어 뷰포트가 표시되는지 확인
    await expect(pageB.getByText("Sync Test")).toBeVisible({
      timeout: 5000,
    });
  });
});
